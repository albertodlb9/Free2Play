...
let idVideojuego = params.get("videojuegoId");
  ...
  fetch("http://localhost:8080/api/usuarios/loged", {
    ...
    .then(data => {
      if (data.message) {
        ...
        fetch("http://localhost:8080/api/reviews/store", {
            method: "POST",
            body: formData,
            credentials: "include"
        })
        ...
        .then(data => {
            console.log(data);
        if (data.error) {
            console.error("Error al crear la review:", data.error);
        } else {
            console.log("review creada correctamente");
            window.location.href = "http://localhost:8080/videojuego?id=" + idVideojuego;
        }
        })
        .catch(error => {
        console.error("Error en la solicitud de creación de la review:", error);
        });
        });
